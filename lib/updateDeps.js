...
const getNextVersion = (pkg) => {
	if (typeof pkg._nextType !== "string") {
		return undefined;
	}

	if (pkg._nextVersion) {
		return pkg._nextVersion;
	}

	const lastVersion = pkg._lastRelease && pkg._lastRelease.version;
	const nextVersion = lastVersion ? semver.inc(lastVersion, pkg._nextType) : "1.0.0";

	pkg._nextVersion = nextVersion;
...
const getDependantRelease = (pkg, bumpStrategy, releaseStrategy, ignore) => {
	...
	return _localDeps
		...
		.reduce((releaseType, p) => {
			...
			const lastVersion = p._lastRelease && p._lastRelease.version;
			const requireRelease = scopes.map((scope) => bumpDependency(scope, name, nextVersion)).some(v => v)
				|| lastVersion && scopes.map((scope) => bumpDependency(scope, name, lastVersion)).some(v => v)
...
/**
 * Resolve dependency next version.
 *
